...
{
    internal class M_Mouvement_caisse
    {
            ...
            try
            {
                if (await Apps.Query.Open())
                {
                    ...
                    if (await Apps.Query.insertPrepared(
                        schema.table["mouvement_caisse"],
                                new MySqlParameter($"@{schema.mouvement_caisse["caisse"]}", args["caisse"]),
                                new MySqlParameter($"@{schema.mouvement_caisse["debut"]}", args["debut"]),
                                 new MySqlParameter($"@{schema.mouvement_caisse["credit"]}", args["credit"]),
                                new MySqlParameter($"@{schema.mouvement_caisse["dates"]}", args["dates"])
                            ))
                    {
            ...
            try
            {
                if (await Apps.Query.Open())
                {
                    ...
                    if (await Apps.Query.updatePrepared(
                             schema.table["mouvement_caisse"],
                             new MySqlParameter($"@{schema.mouvement_caisse["id"]}", args["id"]),
                                new MySqlParameter($"@{schema.mouvement_caisse["caisse"]}", args["caisse"]),
                                new MySqlParameter($"@{schema.mouvement_caisse["debut"]}", args["debut"]),
                                 new MySqlParameter($"@{schema.mouvement_caisse["credit"]}", args["credit"]),
                                new MySqlParameter($"@{schema.mouvement_caisse["dates"]}", args["dates"])
