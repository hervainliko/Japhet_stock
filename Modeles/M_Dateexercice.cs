    ...
    internal class M_Dateexercice
    {
            ...
            try
            {
                if (await Apps.Query.Open())
                {
                    ...
                    if (await Apps.Query.insertPrepared(
                                schema.table["dateexercice"],
                                new MySqlParameter($"@{schema.dateexercice["DateExec"]}", args["DateExec"])
                            ))
                    {
            ...
            try
            {
                if (await Apps.Query.Open())
                {
                    ...
                    if (await Apps.Query.updatePrepared(
                             schema.table["dateexercice"],
                                new MySqlParameter($"@{schema.dateexercice["IDdate"]}", args["IDdate"]),
                                new MySqlParameter($"@{schema.dateexercice["DateExec"]}", args["DateExec"])
                            ))
                    {
                        callback = new Dictionary<string, string> {
        ...
        public async void reseach(string param)
        {
            ...
            try
            {
                ...
                {
                    Apps.Schema schema = new Apps.Schema();
                    Apps.Query.getData($"select * from {schema.table["dateexercice"]} where {schema.dateexercice["DateExec"]} like '%{param}%' order by {schema.dateexercice["IDdate"]} DESC;");
                    callback = new Dictionary<string, string> {
        ...
        public async void get()
        {
            try
            {
                ...
                {
                    Apps.Schema schema = new Apps.Schema();
                    Apps.Query.getData($"select IDdate,date_format(DateExec,'%d-%m-%Y') as DateExec from dateexercice order by IDdate DESC limit 5;");
                    callback = new Dictionary<string, string> {
                        { "type", "success" }, { "message", "Collecte des données sans soucies" }
                    };
                }
        ...
        public async void verifydate(string dates)
        {
            try
            {
                if (await Apps.Query.Open())
                {
                    Apps.Schema schema = new Apps.Schema();
                    Apps.Query.getData($"select COUNT(IDdate) from dateexercice WHERE DateExec='{dates}';");
                    callback = new Dictionary<string, string> {
                        { "type", "success" }, { "message", "Collecte des données sans soucies" }
                    };
                }
        ...
        // Derniere date du systeme
        public async void dernireDate()
        {
            try
            {
                if (await Apps.Query.Open())
                {
                    Apps.Schema schema = new Apps.Schema();
                    Apps.Query.getData($"select IDdate,date(DateExec) FROM dateexercice order by dateexercice.IDdate DESC limit 1");
                    callback = new Dictionary<string, string> {
                        { "type", "success" }, { "message", "Collecte des données sans soucies" }
                    };
                }
        ...
        public async void DescDate(string idadte)
        {
            try
            {
                if (await Apps.Query.Open())
                {
                    Apps.Schema schema = new Apps.Schema();
                    Apps.Query.getData($"select IDdate,date_format(DateExec,'%d-%m-%Y') as DateExec FROM dateexercice WHERE IDdate='" +idadte+"'");
                    callback = new Dictionary<string, string> {
                        { "type", "success" }, { "message", "Collecte des données sans soucies" }
                    };
                }
